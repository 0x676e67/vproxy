...
use tokio::{
    ...
    net::{TcpListener, TcpStream, ToSocketAddrs as AsyncToSocketAddrs, UdpSocket},
    try_join,
};
use tokio_stream::Stream;

use super::{auth::DenyAuthentication, AuthenticationMethod, ReplyError, Socks5Command};
use crate::{
    proxy::{
        auth::Authentication,
        socks5::{
            consts, new_udp_header, parse_udp_request,
            util::{
                addr::{read_address, TargetAddr},
...
pub struct Config<A: Authentication = DenyAuthentication> {
    ...
    /// Enable UDP support
    allow_udp: bool,
    /// For some complex scenarios, we may want to either accept
    /// Username/Password configuration or IP Whitelisting, in case the
    /// client send only 1-2 auth methods (no auth) rather than 3 (with auth)
    allow_no_auth: bool,
...
impl<'a, A: Authentication> Stream for Incoming<'a, A> {
    ...
    /// this code is mainly borrowed from [`Incoming::poll_next()` of
    /// `TcpListener`][tcpListenerLink]
    ///
    ...
    fn poll_next(mut self: Pin<&mut Self>, cx: &mut AsyncContext<'_>) -> Poll<Option<Self::Item>> {
        loop {
            ...
            if let Some(f) = &mut self.1 {
                ...
                tracing::debug!("Incoming connection from peer {peer_addr} @ {local_addr}",);
...
pub struct Socks5Socket<T: AsyncRead + AsyncWrite + Unpin, A: Authentication> {
    ...
    /// If the client has been authenticated, that's where we store his
    /// credentials to be accessed from the socket
...
impl<T: AsyncRead + AsyncWrite + Unpin, A: Authentication> Socks5Socket<T, A> {
    ...
    pub async fn upgrade_to_socks5(mut self) -> Result<Socks5Socket<T, A>> {
        tracing::trace!("Upgrading to socks5...");
    ...
    async fn get_methods(&mut self) -> Result<Vec<u8>> {
        tracing::trace!("Socks5Socket: get_methods()");
        ...
        tracing::debug!("methods supported sent by the client: {:?}", &methods);
    ...
    /// Don't forget that the methods list sent by the client, contains one or
    /// more methods.
    ///
    /// # Request
    ///
        ...
        if let Some(_auth) = self.config.auth.as_ref() {
            if client_methods.contains(&consts::SOCKS5_AUTH_METHOD_PASSWORD) {
                ...
                if self.config.allow_no_auth {
                    ...
                    tracing::debug!("Don't support this auth method, reply with (0xff)");
                    self.inner
                        .write_all(&[
    ...
    async fn read_username_password(socket: &mut T) -> Result<(String, String)> {
        tracing::trace!("Socks5Socket: authenticate()");
        ...
        tracing::debug!("Username bytes: {:?}", &username);

        let [pass_len] = read_exact!(socket, [0u8; 1]).context("Can't read pass len")?;
        tracing::debug!("Auth: [pass len: {len}]", len = pass_len,);
        ...
        tracing::debug!("Password bytes: {:?}", &password);
    ...
    ///  - or the client doesn't send authentication, but we let the trait
    ///    decides if the `allow_no_auth()` set as `true`
    async fn authenticate(&mut self, auth_method: u8) -> Result<A::Item> {
        ...
        if let Some(credentials) = auth.authenticate(credentials).await {
            ...
            tracing::info!("User logged successfully.");
    ...
    /// Wrapper to principally cover ReplyError types for both functions read &
    /// execute request.
    ...
    async fn reply_error(&mut self, error: &ReplyError) -> Result<()> {
        ...
        tracing::debug!("Reply error to be written: {:?}", &reply);
    ...
    async fn read_command(&mut self) -> Result<()> {
        ...
        tracing::debug!(
            "Request: [version: {version}, command: {cmd}, rev: {rsv}, address_type: \
             {address_type}]",
            version = version,
            cmd = cmd,
            rsv = rsv,
        ...
        tracing::debug!("Request target is {}", self.target_addr.as_ref().unwrap());
    ...
    pub async fn resolve_dns(&mut self) -> Result<()> {
        ...
        if let Some(target_addr) = self.target_addr.take() {
            ...
            self.target_addr = match target_addr {
                TargetAddr::Domain(..) => Some(target_addr.resolve_dns().await?),
    ...
    async fn execute_command_connect(&mut self) -> Result<()> {
        ...
        // TCP connect with timeout, to avoid memory leak for connection that takes
        // forever
...
/// Using 2 different generators, because they could be different structs with
/// same traits.
    ...
    O: AsyncRead + AsyncWrite + Unpin,
{
    match tokio::io::copy_bidirectional(&mut inbound, &mut outbound).await {
        Ok(res) => tracing::info!("Transfer closed ({}, {})", res.0, res.1),
        Err(err) => tracing::error!("Transfer error: {:?}", err),
    };
...
// Fixes the issue "cannot borrow data in dereference of `Pin<&mut >` as
// mutable"
//
...
fn new_reply(error: &ReplyError, sock_addr: SocketAddr) -> Vec<u8> {
    ...
    let mut reply = vec![
        consts::SOCKS5_VERSION,
        // transform the error into byte code
        error.as_u8(),
        // reserved
        0x00,
        // address type (ipv4, v6, domain)
        addr_type,
    ];

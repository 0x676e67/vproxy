...
use crate::{
    proxy::socks5::{
        consts::{self, SOCKS5_ADDR_TYPE_IPV4},
        SocksError,
    },
    read_exact,
};
    ...
    pub fn to_be_bytes(&self) -> anyhow::Result<Vec<u8>> {
        ...
        match self {
            TargetAddr::Ip(SocketAddr::V4(addr)) => {
                tracing::debug!("TargetAddr::IpV4");

                buf.extend_from_slice(&[SOCKS5_ADDR_TYPE_IPV4]);

                tracing::debug!("Addr ip {:?}", (*addr.ip()).octets());
                // ip
            ...
            TargetAddr::Domain(ref domain, port) => {
                tracing::debug!("TargetAddr::Domain");
...
impl std::net::ToSocketAddrs for TargetAddr {
    ...
    fn to_socket_addrs(&self) -> io::Result<IntoIter<SocketAddr>> {
        match *self {
            ...
            TargetAddr::Domain(..) => Err(io::Error::new(
                io::ErrorKind::Other,
